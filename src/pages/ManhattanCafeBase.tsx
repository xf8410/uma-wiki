...
const uniqueSkill = {
  nameJP: "アナタヲ・オイカケテ",
  nameCN: "追寻着你",
  nameEN: "Anata wo Oikakete",
  rarity: "独特",
  restriction: "差行",
  iconColor: "黄色",
  descJP: "レース後半に中団から『お友達』を追って速度を少しずつ上げ前方のウマ娘をちょっと萎縮させる",
  descCN: "比赛后半段从中团开始追赶\"朋友\"，逐渐提高速度，并使前方马娘稍微萎缩",
  triggerCode: "distance_rate>=50 &order_rate>=40 &order_rate<=70 &running_style==3",
  triggerCondition: "行进距离≥50%、名次40%~70%、差行",
  skillType: "速度",
  ...
  score: "340",
  ptRatio: "",
  icon: "/images/uma-icons/chara_102501.png",
};
...
const inheritSkill = {
  ...
  restriction: "差行",
  iconColor: "黄色",
  descJP: "レース後半に中団から速度をわずかに上げ続ける",
  descCN: "比赛后半段从中团开始速度略微持续提升",
  triggerCode: "distance_rate>=50 &order_rate>=40 &order_rate<=70 &running_style==3",
  triggerCondition: "行进距离≥50%、名次40%~70%、差行",
  skillType: "速度",
...
export default function ManhattanCafeBase() {
          ...
          <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-start">
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 space-y-6">
              <motion.div variants={itemVariants} className="flex items-center gap-1">
                ...
                <span className="ml-2 text-sm text-[var(--text-muted)]">初始三星 · Creeping Shadow</span>
              </motion.div>
              <motion.div variants={itemVariants} className="space-y-2">
                <p className="text-lg font-semibold" style={{ color: "#6b5ce7", fontFamily: '"Noto Sans JP", sans-serif' }}>マンハッタンカフェ</p>
                <h1 className="text-4xl font-extrabold tracking-tight text-[var(--text-primary)] md:text-5xl">曼城茶座</h1>
                <p className="font-[Poppins] text-lg italic text-[var(--text-muted)]">Manhattan Cafe</p>
              </motion.div>
              ...
              <motion.div variants={itemVariants} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
                <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
                  有着美丽乌黑长发的马少女，性格内向，对初次见面的人说话时语气轻柔。
                  十分喜爱咖啡，但讨厌红茶。经常成为爱丽速子的实验对象。
                  她总是在追逐着某个看不见的身影——那是她唯一的"朋友"。
                </p>
              </motion.div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 40, scale: 0.95 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.3 }} className="relative flex-shrink-0">
              <div className="relative">
                ...
                <div className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
                  <img src={import.meta.env.BASE_URL + "/images/manhattan-base-victory.png"} alt="曼城茶座 - 决胜服" className="h-auto w-72 object-contain sm:w-80 lg:w-96" />
                </div>
                <div className="absolute -bottom-4 -right-4 overflow-hidden rounded-xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl">
                  <img src={import.meta.env.BASE_URL + "/images/manhattan-base-icon.png"} alt="曼城茶座头像" className="h-16 w-16 object-cover sm:h-20 sm:w-20" />
                </div>
              </div>
            </motion.div>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-primary)] p-3 text-center">
                    ...
                    <p className="text-2xl font-bold text-[var(--text-primary)]">持续</p>
                  </div>
                </div>
              </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.6 }} className="fixed bottom-6 left-6 z-50">
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="group relative">
          <div className="h-20 w-20 overflow-hidden rounded-2xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl transition-all group-hover:border-[#6b5ce7] group-hover:shadow-violet-500/20">
            <img src={import.meta.env.BASE_URL + "/images/manhattan-base-chibi.png"} alt="曼城茶座 小人" className="h-full w-full object-contain" />
          </div>
