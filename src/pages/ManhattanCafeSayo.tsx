...
const uniqueSkill = {
  nameJP: "心からのおもてにゃし",
  nameCN: "诚心诚意，以喵待人",
  nameEN: "Kokoro no Omotenashi",
  rarity: "独特",
  restriction: "通用",
  iconColor: "黄色",
  descJP: "レース前半中団以降に控え続けていると\nレース終盤以降かつ最終コーナー以降に\n少し前に出る、長距離レースなら\n効果が増える",
  descCN: "比赛前半段若一直位于队伍中后方\n则在比赛终盘的最终弯道以及往后向前小幅突进\n如果是长距离比赛则增加效果量",
  ...
  score: "340",
  ptRatio: "",
  icon: "/images/manhattan-sayo-skill-unique.png",
  conditions: [
...
const inheritSkill = {
  ...
  score: "180",
  ptRatio: "0.90",
  icon: "/images/manhattan-sayo-skill-inherit.png",
  conditions: [
...
export default function ManhattanCafeSayo() {
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
          ...
          <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-start">
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 space-y-6">
              <motion.div variants={itemVariants} className="flex items-center gap-1">
                ...
                <span className="ml-2 text-sm text-[var(--text-muted)]">初始三星 · 柳緑小夜</span>
              </motion.div>
              <motion.div variants={itemVariants} className="space-y-2">
                <p className="text-lg font-semibold" style={{ color: "#4ade80", fontFamily: '"Noto Sans JP", sans-serif' }}>柳緑小夜</p>
                <h1 className="text-4xl font-extrabold tracking-tight text-[var(--text-primary)] md:text-5xl">柳緑小夜</h1>
                <p className="font-[Poppins] text-lg italic text-[var(--text-muted)]">Yanagi Roku Sayo</p>
              </motion.div>
              ...
              <motion.div variants={itemVariants} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
                <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
                  身着绿白相间和服的曼城茶座。平日阴暗的她换上了清新淡雅的日式旅馆风装扮，
                  仿佛是在温泉小镇中迷路时偶然邂逅的神秘少女。
                  那份追逐前方身影的执着依旧，只是此刻多了几分温柔与宁静。
                </p>
              </motion.div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 40, scale: 0.95 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.3 }} className="relative flex-shrink-0">
              <div className="relative">
                <div className="absolute inset-0 rounded-3xl blur-3xl" style={{ background: "radial-gradient(circle, rgba(74,222,128,0.25) 0%, transparent 70%)" }} />
                <div className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
                  <img src={import.meta.env.BASE_URL + "/images/manhattan-sayo-victory.png"} alt="柳緑小夜 - 决胜服" className="h-auto w-72 object-contain sm:w-80 lg:w-96" />
                </div>
                <div className="absolute -bottom-4 -right-4 overflow-hidden rounded-xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl">
                  <img src={import.meta.env.BASE_URL + "/images/manhattan-sayo-icon.png"} alt="柳緑小夜头像" className="h-16 w-16 object-cover sm:h-20 sm:w-20" />
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.6 }} className="fixed bottom-6 left-6 z-50">
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="group relative">
          <div className="h-20 w-20 overflow-hidden rounded-2xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl transition-all group-hover:border-[#4ade80] group-hover:shadow-green-400/20">
            <img src={import.meta.env.BASE_URL + "/images/manhattan-sayo-chibi.png"} alt="柳緑小夜 小人" className="h-full w-full object-contain" />
          </div>
