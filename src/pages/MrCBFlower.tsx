...
const uniqueSkill = {
  ...
  restriction: "通用",
  iconColor: "黄色",
  descJP: "レース後半に後方にいると残りの持久力に応じたロングスパートをかけて速度をわずかに上げ続ける",
  ...
  score: "340",
  ptRatio: "",
  icon: "/images/mr-cb-flower-skill-unique.png",
};
...
const inheritSkill = {
  ...
  restriction: "通用",
  iconColor: "黄色",
  descJP: "レース後半に後方にいると残りの持久力に応じたロングスパートをかけて速度をほんのわずかに上げ続ける",
  descCN: "比赛后半段位于后方时，根据剩余耐力进行相应时间的长距离冲刺，使速度持续极少量提升",
  triggerCode: "distance_rate>=50 &order_rate>=50",
  triggerCondition: "行进距离≥50%、名次≥50%",
  skillType: "速度",
  ...
  score: "180",
  ptRatio: "0.90",
  icon: "/images/mr-cb-flower-skill-inherit.png",
};
...
export default function MrCBFlower() {
          ...
          <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-start">
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 space-y-6">
              <motion.div variants={itemVariants} className="flex items-center gap-1">
                ...
                <span className="ml-2 text-sm text-[var(--text-muted)]">初始三星 · 花道限定</span>
              </motion.div>
              <motion.div variants={itemVariants} className="space-y-2">
                <p className="text-lg font-semibold" style={{ color: "#e53e3e", fontFamily: '"Noto Sans JP", sans-serif' }}>絢爛花道歌舞く君</p>
                <h1 className="text-4xl font-extrabold tracking-tight text-[var(--text-primary)] md:text-5xl">花道千明代表</h1>
                <p className="font-[Poppins] text-lg italic text-[var(--text-muted)]">Kenran Kadou Kabuku Kimi</p>
              </motion.div>
              ...
              <motion.div variants={itemVariants} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
                <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
                  身着华丽花道服饰的千明代表。以花魁般的优雅姿态在赛道上舞动，
                  她的每一步都仿佛在演绎一场盛大的歌舞伎表演。
                  手中的大毛笔是她的标志，挥洒之间尽显王者风范。
                </p>
              </motion.div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 40, scale: 0.95 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.3 }} className="relative flex-shrink-0">
              <div className="relative">
                ...
                <div className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
                  <img src={import.meta.env.BASE_URL + "/images/mr-cb-flower-victory.png"} alt="花道千明代表 - 决胜服" className="h-auto w-72 object-contain sm:w-80 lg:w-96" />
                </div>
                <div className="absolute -bottom-4 -right-4 overflow-hidden rounded-xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl">
                  <img src={import.meta.env.BASE_URL + "/images/mr-cb-flower-icon.png"} alt="花道千明代表头像" className="h-16 w-16 object-cover sm:h-20 sm:w-20" />
                </div>
              </div>
            </motion.div>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-primary)] p-3 text-center">
                    ...
                    <p className="text-2xl font-bold text-[var(--text-primary)]">{uniqueSkill.duration}</p>
                  </div>
                </div>
              </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.6 }} className="fixed bottom-6 left-6 z-50">
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="group relative">
          <div className="h-20 w-20 overflow-hidden rounded-2xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl transition-all group-hover:border-[#e53e3e] group-hover:shadow-red-500/20">
            <img src={import.meta.env.BASE_URL + "/images/mr-cb-flower-chibi.png"} alt="花道千明代表 小人" className="h-full w-full object-contain" />
          </div>
