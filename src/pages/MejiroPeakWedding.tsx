...
const basicInfo = [
  { icon: Mic, label: "CV", value: "高柳知叶" },
  { icon: Calendar, label: "生日", value: "4月3日" },
  { icon: Ruler, label: "身高", value: "167cm" },
...
const uniqueSkill = {
  ...
  restriction: "先行",
  iconColor: "黄色",
  descJP: "残り300m地点で前方にいると\n速度が少し上がる\nスキル発動時、先頭か先頭から2バ身\n以内だとすごく上がる＜作戦・先行＞",
  descCN: "剩余300m的位置位于前方时\n速度少量提升，发动技能时\n若位于领头或距离领头2马身以内\n则速度大幅提升＜作战・先行＞",
  ...
  score: "340",
  ptRatio: "",
  icon: "/images/mejiro-peak-wedding-skill-unique.png",
  conditions: [
...
const inheritSkill = {
  ...
  score: "180",
  ptRatio: "0.90",
  icon: "/images/mejiro-peak-wedding-skill-inherit.png",
  conditions: [
...
const fieldData = [
  { label: "草地", grade: "A" }, { label: "泥地", grade: "F" },
];
const distanceData = [
  { label: "短距离", grade: "B" }, { label: "英里", grade: "A" },
  { label: "中距离", grade: "A" }, { label: "长距离", grade: "E" },
];
const strategyData = [
  { label: "逃", grade: "G" }, { label: "先行", grade: "A" },
  { label: "差行", grade: "A" }, { label: "追", grade: "F" },
];
...
export default function MejiroPeakWedding() {
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
          ...
          <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-start">
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 space-y-6">
              <motion.div variants={itemVariants} className="flex items-center gap-1">
                ...
                <span className="ml-2 text-sm text-[var(--text-muted)]">初始三星 · 婚纱</span>
              </motion.div>
              <motion.div variants={itemVariants} className="space-y-2">
                <p className="text-lg font-semibold" style={{ color: "#f5deb3", fontFamily: '"Noto Sans JP", sans-serif' }}>ウェディング・ラモーヌ</p>
                <h1 className="text-4xl font-extrabold tracking-tight text-[var(--text-primary)] md:text-5xl">婚纱目白高峰</h1>
                <p className="font-[Poppins] text-lg italic text-[var(--text-muted)]">Wedding Ramonu</p>
              </motion.div>
              ...
              <motion.div variants={itemVariants} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
                <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
                  身着纯白婚纱的目白高峰。金色刺绣点缀的白色礼服如同梦幻般优雅，
                  平日里温柔沉稳的她此刻散发着新娘般的神圣光芒。
                  至死不渝的誓言化作战场上的力量，为爱而奔跑的身影令人动容。
                </p>
              </motion.div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 40, scale: 0.95 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.3 }} className="relative flex-shrink-0">
              <div className="relative">
                <div className="absolute inset-0 rounded-3xl blur-3xl" style={{ background: "radial-gradient(circle, rgba(245,222,179,0.25) 0%, transparent 70%)" }} />
                <div className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
                  <img src={import.meta.env.BASE_URL + "/images/mejiro-peak-wedding-victory.png"} alt="婚纱目白高峰 - 决胜服" className="h-auto w-72 object-contain sm:w-80 lg:w-96" />
                </div>
                <div className="absolute -bottom-4 -right-4 overflow-hidden rounded-xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl">
                  <img src={import.meta.env.BASE_URL + "/images/mejiro-peak-wedding-icon.png"} alt="婚纱目白高峰头像" className="h-16 w-16 object-cover sm:h-20 sm:w-20" />
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.6 }} className="fixed bottom-6 left-6 z-50">
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="group relative">
          <div className="h-20 w-20 overflow-hidden rounded-2xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl transition-all group-hover:border-[#f5deb3] group-hover:shadow-amber-200/20">
            <img src={import.meta.env.BASE_URL + "/images/mejiro-peak-wedding-chibi.png"} alt="婚纱目白高峰 小人" className="h-full w-full object-contain" />
          </div>
