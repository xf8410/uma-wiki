...
const basicInfo = [
  { icon: Mic, label: "CV", value: " Lynn" },
  { icon: Calendar, label: "生日", value: "5月6日" },
  { icon: Ruler, label: "身高", value: "157cm" },
...
const uniqueSkill = {
  nameJP: "ときめきが呼ぶほうへ",
  nameCN: "奔向那名为悸动的彼方",
  nameEN: "Tokimeki ga Yobu Hou e",
  rarity: "独特",
  restriction: "通用",
  iconColor: "黄色",
  descJP: "終盤目前の下り坂で中団に控えていて\nゴールまで遠いとレースを\n思いっきり楽しんで速度を上げる",
  descCN: "在临近终盘的下坡路处于中间集团附近，且距离终点尚远时，\n会尽情地享受比赛，提高速度",
  ...
  score: "340",
  ptRatio: "",
  icon: "/images/mejiro-dober-summer-skill-unique.png",
};
...
const inheritSkill = {
  ...
  score: "180",
  ptRatio: "0.90",
  icon: "/images/mejiro-dober-summer-skill-inherit.png",
};
...
const distanceData = [
  { label: "短距离", grade: "E" }, { label: "英里", grade: "A" },
  { label: "中距离", grade: "A" }, { label: "长距离", grade: "F" },
];
const strategyData = [
  { label: "逃", grade: "C" }, { label: "先行", grade: "B" },
  { label: "差行", grade: "A" }, { label: "追", grade: "G" },
];
...
export default function MejiroDoberSummer() {
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
          ...
          <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-start">
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 space-y-6">
              <motion.div variants={itemVariants} className="flex items-center gap-1">
                ...
                <span className="ml-2 text-sm text-[var(--text-muted)]">初始三星 · 夏日度假</span>
              </motion.div>
              <motion.div variants={itemVariants} className="space-y-2">
                <p className="text-lg font-semibold" style={{ color: "#3b82f6", fontFamily: '"Noto Sans JP", sans-serif' }}>バカンス・サフィール</p>
                <h1 className="text-4xl font-extrabold tracking-tight text-[var(--text-primary)] md:text-5xl">夏装目白多伯</h1>
                <p className="font-[Poppins] text-lg italic text-[var(--text-muted)]">Vacance Saphir</p>
              </motion.div>
              ...
              <motion.div variants={itemVariants} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
                <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
                  换上夏日度假装的目白多伯。宽檐草帽搭配深蓝色露肩连衣裙，
                  平时冷静的她难得露出放松的一面。
                  在度假胜地享受夏日时光的同时，赛道上那份追求胜利的悸动依然不曾改变。
                </p>
              </motion.div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 40, scale: 0.95 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.3 }} className="relative flex-shrink-0">
              <div className="relative">
                <div className="absolute inset-0 rounded-3xl blur-3xl" style={{ background: "radial-gradient(circle, rgba(59,130,246,0.25) 0%, transparent 70%)" }} />
                <div className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
                  <img src={import.meta.env.BASE_URL + "/images/mejiro-dober-summer-victory.png"} alt="夏装目白多伯 - 决胜服" className="h-auto w-72 object-contain sm:w-80 lg:w-96" />
                </div>
                <div className="absolute -bottom-4 -right-4 overflow-hidden rounded-xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl">
                  <img src={import.meta.env.BASE_URL + "/images/mejiro-dober-summer-icon.png"} alt="夏装目白多伯头像" className="h-16 w-16 object-cover sm:h-20 sm:w-20" />
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 0.6 }} className="fixed bottom-6 left-6 z-50">
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="group relative">
          <div className="h-20 w-20 overflow-hidden rounded-2xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl transition-all group-hover:border-[#3b82f6] group-hover:shadow-blue-500/20">
            <img src={import.meta.env.BASE_URL + "/images/mejiro-dober-summer-chibi.png"} alt="夏装目白多伯 小人" className="h-full w-full object-contain" />
          </div>
