...
const basicInfo = [
  { icon: Mic, label: "CV", value: "高桥未奈美" },
  { icon: Calendar, label: "生日", value: "3月21日" },
  { icon: Ruler, label: "身高", value: "171cm" },
...
const uniqueSkill = {
  nameJP: "叙情、旅路の果てに",
  nameCN: "旅途终点的抒情诗",
  nameEN: "Jojo, Tabiji no Hate ni",
  rarity: "独特",
  restriction: "通用",
  iconColor: "黄色",
  descJP: "終盤が迫ったどこかで後方にいると速度を少し上げ続ける。中距離か長距離レースで先頭から離れていると効果が増える",
...
const inheritSkill = {
  ...
  restriction: "通用",
  iconColor: "黄色",
  descJP: "終盤が迫ったどこかで後方にいると速度をちょっと上げ続ける。中距離か長距離レースで先頭から離れていると効果が増える",
...
const distanceData = [
  { label: "短距离", grade: "G" }, { label: "英里", grade: "B" },
  { label: "中距离", grade: "A" }, { label: "长距离", grade: "A" },
];
const strategyData = [
  { label: "逃", grade: "G" }, { label: "先行", grade: "E" },
  { label: "差行", grade: "A" }, { label: "追", grade: "A" },
];
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
          ...
          <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-start">
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 space-y-6">
              ...
              <motion.div variants={itemVariants} className="space-y-2">
                <p className="text-lg font-semibold" style={{ color: "var(--accent-purple)", fontFamily: '"Noto Sans JP", sans-serif' }}>
                  ミスターシービー
                </p>
              ...
              <motion.div variants={itemVariants} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-4">
                <p className="text-sm leading-relaxed text-[var(--text-secondary)]">
                  自由奔放的赛马娘，以华丽的跑法征服赛场。
                  总是保持着从容不迫的态度，但在关键时刻会展现出惊人的集中力。
                  有着"千名の代表"（千名代表）的称号，象征着她在赛场上的统治力。
                </p>
              </motion.div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 40, scale: 0.95 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.3 }} className="relative flex-shrink-0">
              <div className="relative">
                {/* Glow */}
                <div className="absolute inset-0 rounded-3xl blur-3xl" style={{ background: "radial-gradient(circle, rgba(139,92,246,0.25) 0%, transparent 70%)" }} />
                <div className="relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)]">
                  <img src={import.meta.env.BASE_URL + "/images/mr-cb-victory.png"} alt="千名代表 - 决胜服" className="h-auto w-72 object-contain sm:w-80 lg:w-96" />
                </div>
                {/* Icon Badge */}
                <div className="absolute -bottom-4 -right-4 overflow-hidden rounded-xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl">
                  <img src={import.meta.env.BASE_URL + "/images/mr-cb-icon.png"} alt="千名代表头像" className="h-16 w-16 object-cover sm:h-20 sm:w-20" />
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="flex flex-col gap-6 md:flex-row">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="flex flex-shrink-0 flex-col items-center gap-3">
              <div className="overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] p-1">
                <img src={import.meta.env.BASE_URL + "/images/mr-cb-skill-unique.png"} alt={uniqueSkill.nameJP} className="h-24 w-24 object-contain sm:h-28 sm:w-28" />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {uniqueSkill.conditions.map((c) => (
                    <div key={c.label} className={`rounded-xl border ${c.special ? "border-yellow-500/60" : "border-[var(--border-subtle)]"} bg-[var(--bg-primary)] p-4`}>
                      <div className="mb-2 flex items-center gap-2">
                        <span className="text-sm font-semibold text-[var(--accent-pink)]">{c.label}</span>
                        {c.special && <Badge variant="secondary" className="h-4 px-1 text-[9px] border-yellow-500/30 bg-yellow-500/20 text-yellow-300">特殊效果</Badge>}
                      </div>
                      ...
                      <div className="flex items-center gap-2 border-t border-[var(--border-subtle)] pt-2">
                        ...
                        <span className="text-xs text-[var(--text-muted)]">持续: </span>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6 }} className="card-hover relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)] p-6 sm:p-8" style={{ borderTop: "4px solid var(--accent-pink)" }}>
          ...
          <div className="flex flex-col gap-6 md:flex-row">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="flex flex-shrink-0 flex-col items-center gap-3">
              <div className="overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] p-1">
                <img src={import.meta.env.BASE_URL + "/images/mr-cb-skill-inherit.png"} alt={`继承·${inheritSkill.nameJP}`} className="h-24 w-24 object-contain sm:h-28 sm:w-28" />
              </div>
      ...
      >
        ...
        >
          <div className="h-20 w-20 overflow-hidden rounded-2xl border-2 border-[var(--border-subtle)] bg-[var(--bg-secondary)] shadow-xl transition-all group-hover:border-[var(--accent-purple)] group-hover:shadow-purple-500/20">
            <img src={import.meta.env.BASE_URL + "/images/mr-cb-chibi.png"} alt="千名代表 小人" className="h-full w-full object-contain" />
          </div>
